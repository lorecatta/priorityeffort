...
###
# function for creating a list of all the upstream sites to each site - connections are real and virtual 
###
get.upstream_connections <- function(boundary.file, site_species_array)
{
  no.sites <- nrow(site_species_array)
  no.links <- max(unique(boundary.file[,"links"]))
  
  all_upstream_connections <- vector("list", no.sites)

  for (i in 1:no.sites)
  {
    
    if (any(boundary.file[,"id1"]==i))
    {
      
      tiny_list_1 <- vector("list", no.links)
      
      for(j in 1:no.links)
      {
        
        upstream_connections <- which(boundary.file[,"id1"]==i & boundary.file[,"links"]==j)
        tiny_list_1[[j]] <- upstream_connections
     
      }
      
      all_upstream_connections [[i]] <- unlist(tiny_list_1)
...
###
# function for creating a list of all the downstream sites to each site - connections are real and virtual
###
get.downstream_connections <- function(boundary.file, site_species_array)
{
  no.sites <- nrow(site_species_array)
  no.links <- max(unique(boundary.file[,"links"]))
  
  all_downstream_connections <- matrix(0, ncol=no.sites, nrow=no.links)
    
  all_downstream_connections_list <- vector("list", no.sites)
  ...
  for (i in 1:no.links)
  {
    ...
    for(j in 1:no.sites) 
    {
      ...
      Check <- which(boundary.file[,"id2"]==j & boundary.file[,"links"]==i)
      
      if(length(Check)>0)
      {
        
        all_downstream_connections[i,j] <- Check  
  ...
  for (i in 1:no.sites)
  {
    ...
    all_downstream_connections_list [[i]] <- all_downstream_connections[,i] [all_downstream_connections[,i] !=0]
  
  }

  all_downstream_connections_list
}
...
set_scaled_targets <- function(cons_feat_array_df, occurrence_limits, target_limits){
  
  # species targets are linearly scaled to species' area of occupancy 
  
  cons_feat_array_df$target_perc <- 0
  
  below_lim_1 <- which(cons_feat_array_df$area_of_occ < occurrence_limits[1])
  above_lim_2 <- which(cons_feat_array_df$area_of_occ > occurrence_limits[2])
  
  cons_feat_array_df[below_lim_1, "target_perc"] <- target_limits[1] * 100
  cons_feat_array_df[above_lim_2, "target_perc"] <- target_limits[2] * 100
  
  cons_feat_array_df[below_lim_1, "target"] <- round(cons_feat_array_df[below_lim_1, "area_of_occ"] * target_limits[1], digits = 2)
  cons_feat_array_df[above_lim_2, "target"] <- round(cons_feat_array_df[above_lim_2, "area_of_occ"] * target_limits[2], digits = 2)
  
  between_lims_ids <- which(cons_feat_array_df$target == 0) 
  between_lims <- cons_feat_array_df[between_lims_ids, ]
  
  interp_targ <- vapply(between_lims$area_of_occ, 
                        function(i, a, b) approx(a, b, xout = i)$y,
                        numeric(1),
                        a = occurrence_limits,
                        b = target_limits)
  
  cons_feat_array_df[between_lims_ids, "target_perc"] <- interp_targ * 100
  
  cons_feat_array_df[between_lims_ids, "target"] <- round(cons_feat_array_df[between_lims_ids, "area_of_occ"] * interp_targ, digits = 2)
  
  cons_feat_array_df <- cbind(ID = seq_len(nrow(cons_feat_array_df)), cons_feat_array_df)
  
  write.csv(cons_feat_array_df, 
            file.path("output", paste0("output_exp_", parameters$Exp), "species_targets.csv"), 
            row.names = FALSE)
