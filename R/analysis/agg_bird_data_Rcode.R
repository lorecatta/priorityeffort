Species_Daly_final_inters <- read.table('C:/Users/s2872641/Documents/Griffith_post_doc/Data_Daly/Species_Daly/Birds_Daly_final_inters.txt', header=TRUE, sep = ",")
Species_Daly_final_inters <- Species_Daly_final_inters[,c(2,5,31:117)]
Species_Daly_final_inters <- as.matrix(Species_Daly_final_inters)

#select a size threshold (sq km) to remove smaller areas
larger_areas_indices <- which(Species_Daly_final_inters [,2]>=3)
smaller_areas_indices <- which(Species_Daly_final_inters [,2]<3)
...
mat <- matrix(0, ncol=88, nrow=865)
colnames(mat) <- 1:dim(mat)[2]
col_names <- colnames(dataset)
col_names[2] <- 'PU_ID'
col_names <- col_names[2:89]
...
write.table(agg_larger_areas,'C:/Users/s2872641/Documents/Griffith_post_doc/Data_Daly/Species_Daly/agg_bird_data.txt',sep = ",", col.names = NA)
